...
Virksomheder med begrænset kontakt til organisationen, lav deltagelse i arrangementer og uden dokumenteret interaktion har generelt en markant højere risiko for at opsige deres medlemskab. Dette underbygges af figuren nedenfor, der viser de fem postnumre med den højeste gennemsnitlige churn-risiko. Her ses, at geografiske områder med lav tilknytning til det centrale område udviser særlig høj churn-sandsynlighed.

![Top 5 postnumre med højeste churn risiko](images/6_postnummer_højeste_churn.png){ width=80% fig-align="center" }

Også på brancheniveau er der tydelige forskelle. Nogle brancher er kendetegnet ved lav netværksværdi og begrænset interaktion med organisationen og har derfor en højere sandsynlighed for medlemsophør. Figuren herunder visualiserer de fem brancher med den højeste gennemsnitlige churn-risiko, hvilket bekræfter tendensen.

![Top 5 brancher med højeste churn sandsynlighed](images/5_brancher_højeste_churn.png){ width=80% fig-align="center" }
...
![Top 5 brancher hvor sandsynligheden for churn er lav](images/7_top5_der_ikke_churner.png){ width=80% fig-align="center" }
...
![Model performance – Accuracy, F1 og AUC](images/1_model_performance.png){ width=80% fig-align="center" }
...
![Performance heatmap](images/3_heatmap_pr_model.png){ width=80% fig-align="center" }

![Linjeplot med sammenligning af metrikker](images/2_linje_plot.png){ width=80% fig-align="center" }
...
![Top 10 variabler (Random Forest)](images/8_top_10_variabler_rf.png){ width=80% fig-align="center" }
...
# 1. Ekstraher den trænede Random Forest-model fra workflowet
final_rf_fit <- extract_fit_parsnip(final_model)

# 2. Brug vip til at finde de 10 vigtigste variabler
vip_rf <- vi(final_rf_fit) |> 
  slice_max(order_by = Importance, n = 10) |> 
  mutate(Variable = str_wrap(Variable, width = 30))

# 3. Plot top 10 vigtigste variabler
