      ...
      tabsetPanel(
        tabPanel(
          ...
          absolutePanel(
            ...
            draggable = TRUE,
            style = "background: white; padding: 10px; border-radius: 5px; box-shadow: 0 1px 5px rgba(0,0,0,0.1);",
            h5("Map Legend"),
            tags$div(
              HTML('<i class="fa fa-circle" style="color:#e74c3c"></i> High Risk (75-100%)<br>
                   <i class="fa fa-circle" style="color:#f39c12"></i> Medium Risk (50-75%)<br>
                   <i class="fa fa-circle" style="color:#2ecc71"></i> Low Risk (0-50%)')
            ),
            sliderInput(
              "marker_size",
              "Marker Size:",
        ...
        tabPanel(
          "Summary Statistics",
          h4("Churn Risk Distribution"),
          plotOutput("risk_distribution", height = "300px"),
          h4("Risk by Postal Code"),
...
server <- function(input, output, session) {
  ...
  # Update selectize input for member IDs server-side
  ...
  filtered_data <- reactive({
    ...
    # Apply churn probability filter
    data <- data %>%
      filter(churn_prob >= input$churn_range[1],
  ...
  # Create map (simplified without leaflet.extras)
    ...
    leaflet(df) %>%
      ...
      addCircleMarkers(
        ...
        popup = ~paste0(
          ...
          "<b>Churn Risk:</b> <span class='risk-", tolower(risk_category), "'>", 
          round(churn_prob * 100, 1), "%</span><br>",
          "<b>Category:</b> ", risk_category, "<br>",
          "<b>Coordinates:</b> ", round(lat, 4), ", ", round(lng, 4)
        ),
  ...
  output$data_table <- renderDT({
    datatable(
      filtered_data() %>%
        select(PNumber, PostalCode, churn_prob, risk_category, lat, lng),
      rownames = FALSE,
  ...
  output$download_data <- downloadHandler(
    filename = function() {
      paste("churn_data_", Sys.Date(), ".csv", sep = "")
    },
