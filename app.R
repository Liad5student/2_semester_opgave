...
# 1. FORBEREDELSE - Load packages and data
library(shiny)
library(leaflet)
library(dplyr)
library(readr)
...
# In the UI section, replace the Business Viborg conditionalPanel with:
conditionalPanel(
  condition = "input.main_tabs == 'dashboard'",
  div(
    h4("Om Business Viborg"),
    ...
    p(strong("Adresse:"), br(), "Erik Ejegods Vej 16, 2. sal", br(), "8800 Viborg"),
...
server <- function(input, output, session) {
  ...
  filtered_data <- reactive({
    req(input$postal_code, input$CompanyTypeName, input$churn_range)
  ...
  risk_pal <- c("Low" = "#2ca02c", "Medium" = "#ffcc00", "High" = "#d62728")
  ...
  observeEvent(input$reset_filters, {
    updateSliderInput(session, "churn_range", value = c(0, 100))
    ...
    updatePickerInput(session, "CompanyTypeName", selected = character(0))
    updatePickerInput(session, "Branche_navn", selected = character(0))
  })
  ...
  output$simulation_result <- renderUI({
    if (!is.null(simulation_result())) {
      ...
      color <- ifelse(res$risk_category == "High", "#d9534f",
                      ifelse(res$risk_category == "Medium", "#f0ad4e", "#5cb85c"))
      
      div(
        style = "margin-bottom: 30px;",
        h3(paste0("Churn-sandsynlighed: ", round(res$churn_prob * 100, 1), "%"), 
           style = paste0("color: ", color, "; text-align: center;")),
        h4(paste0("Risikokategori: ", res$risk_category), 
  ...
  output$simulation_gauge <- renderPlot({
    if (!is.null(simulation_result())) {
      ...
      ggplot() +
        ...
        geom_arc_bar(aes(x0 = 0, y0 = 0, r0 = 0.5, r = 1,
                         start = 0, end = 2*pi * res$churn_prob, 
                         fill = color)) +
        geom_text(aes(x = 0, y = 0, 
                      label = paste0(round(res$churn_prob * 100, 1), "%")),
